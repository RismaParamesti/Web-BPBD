...
const OfficeEdit = () => {
  ...
  const [form, setForm] = useState({
    ...
    region_id: "", // id region dari Office
    subdistrict_id: "", // id subdistrict dari Office
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(`https://api.satudatabidangpk.com/offices/${id}`);
        const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await axios.put(`https://api.satudatabidangpk.com/offices/${id}`, formData, {
          headers: {
        ...
        const res = await axios.post(
          "https://api.satudatabidangpk.com/offices",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
      ...
      navigate(`${basePath}/Office/Detail/${id}`);
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-base-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-primary mt-4">
          Edit Data <span className="text-secondary">Office</span>
        </h1>
      </div>
