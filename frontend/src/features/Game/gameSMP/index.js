...
const GameSMP = () => {
  ...
  const [gameSMPId, setGameSMPId] = useState(""); // untuk link explore
  ...
  useEffect(() => {
    ...
    const fetchGameData = async () => {
     try {
        const res = await axios.get("https://api.satudatabidangpk.com/games/3");
      ...
      <div className="bg-base-100 p-4 sm:p-6 rounded-xl shadow-lg">
        ...
        {gameSMPId && (
          <div className="text-left mt-4">
            <a
              href={`https://quizizz.com/admin/quiz/${gameSMPId}`}
              target="_blank"
         ...
         {(role === "admin" || role === "superadmin") && (
          <div className="flex justify-end mt-6">
            <button
              onClick={() => navigate(`${basePath}/Game/Edit/${3}`)} 
